...
def serve():
    # Создается экземпляр gRPC-сервера.
    # пул рабочих потоков, который будет использоваться для обработки входящих запросов. 
    # В данном случае пул содержит 10 рабочих потоков.
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=10))

    # Добавляется сервис ServerResponceServicer к серверу.
    # ServerResponceServicer - это класс, который реализует логику обработки запросов на сервере.
    ship_pb2_grpc.add_ServerResponceServicer_to_server(ServerResponceServicer(), server)
    # Серверу добавляется не защищенный порт для прослушивания входящих запросов.
